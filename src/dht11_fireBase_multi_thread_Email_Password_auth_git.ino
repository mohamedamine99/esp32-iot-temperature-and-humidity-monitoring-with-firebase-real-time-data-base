...
#define DATABASE_URL "xxxxxxxxxxxxxxxxxxxxxx.firebaseio.com" //<databaseName>.firebaseio.com or <databaseName>.<region>.firebasedatabase.app
#define USER_EMAIL "xxxxxxxxxxxxxxxxxxxxxxxx"
...
if (Sensor_Error_Code!= DHTLIB_WAITING_FOR_READ) {
...
else if (temperature<temp_min_threshhold)
{
 ...
 switch (Sensor_Error_Code)
  ...
  Serial.println("****************************");
   Serial.println(SensorState.c_str());
...
for(;;)
  {
        ...
        Serial.println("****updating reached 1**** ");
        Update_data(); 
      //  yield();
        Serial.println("**************** updating reached 2 ****************** ");   
        delay(2000) ;   
        yield();
